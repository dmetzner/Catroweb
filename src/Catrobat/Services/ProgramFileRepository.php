...
class ProgramFileRepository
{
  ...
  public function saveProgram(ExtractedCatrobatFile $extracted, string $id)
  {
    $this->file_compressor->compress($extracted->getPath(), $this->directory, $id);
  }

  public function saveProgramTemp(ExtractedCatrobatFile $extracted, string $id)
  {
    if ($this->tmp_dir)
    {
  ...
  public function makeTempProgramPerm(string $id)
  {
    if ($this->tmp_dir)
    {
  ...
  public function deleteProgramFile(string $id)
  {
    $this->filesystem->remove($this->directory.$id.'.catrobat');
  }

  public function saveProgramFile(File $file, string $id)
  {
