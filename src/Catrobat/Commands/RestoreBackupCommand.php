  ...
  protected function execute(InputInterface $input, OutputInterface $output): void
  {
    ...
    if ($input->hasArgument('local') && 'true' === $input->getArgument('local'))
    {
      ...
      CommandHelper::executeShellCommandLegacy(
        "time pigz -dc {$backup_file} | tar -xf - -C {$local_resource_directory}", ['timeout' => 86400],
    ...
    else
    {
      ...
      CommandHelper::executeShellCommandLegacy(
        ...
        '"php '.$backup_host_directory.'bin/console catrobat:purge --env=prod --force" ', ['timeout' => 86400],
        'Remove files from server resources directory', $output
      );
      CommandHelper::executeShellCommandLegacy(
        "gzip -dc {$backup_file} | ".
        "sshpass -p '{$backup_host_password}' ssh {$backup_host_user}@{$backup_host_name} ".
        'tar -xf - -C '.$backup_host_directory.$backup_host_resource_directory.' ', ['timeout' => 86400],
        'Extract files to server resources directory', $output
      );
      CommandHelper::executeShellCommandLegacy(
        "sshpass -p '{$backup_host_password}' ssh {$backup_host_user}@{$backup_host_name} ".
        "\"mysql -u {$backup_database_user} -p{$backup_database_password} {$backup_database_name} ".
        '< '.$backup_host_directory.$backup_host_resource_directory.'database.sql" ', ['timeout' => 86400],
        'Restore SQL file', $output
      );
      CommandHelper::executeShellCommandLegacy(
        "sshpass -p '{$backup_host_password}' ssh {$backup_host_user}@{$backup_host_name} ".
        '"unlink '.$backup_host_directory.$backup_host_resource_directory.'database.sql" ', ['timeout' => 86400],
        'Remove files from server resources directory', $output
      );
      CommandHelper::executeShellCommandLegacy(
        "sshpass -p '{$backup_host_password}' ssh {$backup_host_user}@{$backup_host_name} ".
